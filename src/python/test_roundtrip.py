...
with SwimFile(path, "w") as f:
    ...
    f.add_laps(
        lap_number=[1, 2],
        lap_time_s=[25.10, 27.21],
        stroke_count=[32, 34],
        stroke_rate_spm=[52.3, 50.1],
        stroke_length_m=[2.1, 2.0],
    )
    f.add_kinematics(
        time=t, joint_names=joints, position=pos, velocity=vel, sample_rate_hz=fps
    )
    f.add_stroke_metrics(
        stroke_index=np.arange(66),
        stroke_time_s=np.linspace(0, 52.31, 66),
    ...
    f.add_imu(
        location="sacrum",
        time=t,
        accel=np.random.randn(n_frames, 3),
        gyro=np.random.randn(n_frames, 3),
        sample_rate_hz=fps,
    )
    f.add_forces(
        time=np.linspace(0, 0.4, 400), force_n=np.random.uniform(200, 600, 400)
    )
    f.add_video_sync(
        sync_timestamps=t[::4],
        frame_rate_hz=25.0,
        resolution="1920x1080",
        camera_id="cam_side_1",
    )
